...
function solve_transient!(
    ...
    t1 = @elapsed for i in 1:(length(lambdas) - 1)
        ...
        elseif control.timestep_mode == :fixed
            nfloat = (λend - λ) / Δλ
            nround = round(Int, nfloat)
            if isapprox(nfloat, nround; atol = 1.0e-12, rtol = 1.0e-8)
                nsteps = max(1, nround)
            else
                nsteps = max(1, ceil(Int, nfloat))
                @warn "Fixed timestep adjusted on interval [$λstart,$λend]: requested Δt=$Δλ gives non-integer step count $(nfloat). Using $nsteps equal steps with Δt=$((λend-λ)/nsteps)."
            end
            Δλ = (λend - λ) / nsteps
        end
